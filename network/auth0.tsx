...
async function fetchAccessToken(
    ...
    if (tokenResponse.ok) {
        ...
        AsyncStorage.setItem("accessToken", token.access_token);
        if(token.refresh_token){
...
export function Auth0Provider({
    ...
    const logout = async () => {
        const redirectU = AuthSession.makeRedirectUri({ useProxy: false });
        console.log("gettoken cagrildi", redirectU);
        const logoutUrl = 'https://' + domain + `/v2/logout?client_id=${clientId}&returnTo=${redirectU}`;
        try {
    ...
    useEffect(() => {
        async function setTokenAtBeginning() {
            if(auth0request){
            const accessT = await AsyncStorage.getItem("accessToken");
            console.log("setToken", accessT, domain)   
            try{
                console.log("eee",auth0request)
            const userInfoResponse = await fetch(`https://${domain}/userinfo`, {
            ...
            console.log("try>")
                ...
                // Set state at the same time to trigger a single update on the context
                // otherwise components are sent two separate updates
                console.log("userinfoo", userInfoResponse)
